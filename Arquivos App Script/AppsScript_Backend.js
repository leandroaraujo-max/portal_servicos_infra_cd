...
/**
 * Retorna lista de servidores do Cluster WMS a partir da aba 'Servidores' (Coluna C).
 * Usado pelo Daemon para failover e sync.
 */
function getWmsClusterNodes() {
    const ss = SpreadsheetApp.getActiveSpreadsheet(); // Tenta pegar a ativa ou usa getDatabaseConnection
    // Como getDatabaseConnection() pode ser complexa, vou assumir o SS ativo já que o script roda vinculado.
    // Mas o ideal é usar a mesma lógica do restante.
    // Vou usar getDatabaseConnection() se disponível no escopo, ou SpreadsheetApp.openById...
    // Vou usar o helper getDatabaseConnection() que já existe no script.

    try {
        const conn = getDatabaseConnection();
        // Tenta aba 'Servidores' (ou 'Cluster', 'WMS')
        let sheet = conn.getSheetByName("Servidores");

        if (!sheet) {
            console.warn("Aba 'Servidores' não encontrada. Usando fallback hardcoded.");
            return [
                "ml-ibm-wms-01.magazineluiza.intranet",
                "ml-ibm-wms-02.magazineluiza.intranet",
                "ml-ibm-wms-03.magazineluiza.intranet",
                "ml-ibm-wms-04.magazineluiza.intranet"
            ];
        }

        const lastRow = sheet.getLastRow();
        if (lastRow < 2) return ["ml-ibm-wms-01.magazineluiza.intranet"];

        // Coluna C = Index 3 (1-based) -> getRange(2, 3, l, 1)
        const data = sheet.getRange(2, 3, lastRow - 1, 1).getValues();
        const servers = [];

        for (let i = 0; i < data.length; i++) {
            const server = String(data[i][0]).trim();
            if (server && server.length > 5 && !servers.includes(server)) {
                servers.push(server);
            }
        }

        return servers.length > 0 ? servers : ["ml-ibm-wms-01.magazineluiza.intranet"];

    } catch (e) {
        console.error("Erro em getWmsClusterNodes:", e);
        return ["ml-ibm-wms-01.magazineluiza.intranet"];
    }
}
