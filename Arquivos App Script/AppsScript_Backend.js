...
function doGet(e) {
    // 1. Rota de Aprovação (Link enviado por email)
    if (e.parameter && e.parameter.action === 'approve') {
        if (!e.parameter.email) {
            return ContentService.createTextOutput("Erro: Parâmetro 'email' ausente.");
        }
        const result = approveUser(e.parameter.email);
        return ContentService.createTextOutput(result);
    }

    // 2. Rota Padrão (Carrega a Aplicação Web)
    ...
    return tmp.evaluate()
        .setTitle('Gerenciamento de Usuários - Suporte Infra CDs')
...
function doGet(e) {
    ...
    if (e && e.parameter && e.parameter.mode === 'get_analysts') {
        return ContentService.createTextOutput(JSON.stringify(getAnalystsList()))
            .setMimeType(ContentService.MimeType.JSON);
    }

    // 3. API POWERSHELL (FILA DE ESPELHO)
    if (e && e.parameter && e.parameter.mode === 'check_mirror_queue') {
        return handleMirrorQueueCheck();
    }

    // 4. API POWERSHELL (FILA DE RESET)
    if (e && e.parameter && e.parameter.mode === 'api') {
        return handlePowerShellQueueRequest();
    }

    // 5. WEB APP (INTERFACE PADRÃO)
...
function doPost(e) {
    try {
        ...
        // 1. LÓGICA DE ATUALIZAÇÃO DO AGENTE (FETCH_GROUPS)
        ...
        try {
            ...
            if (sheetFila) {
                ...
                const idReset = String(data.id_solicitacao).trim();
                for (let i = 1; i < dataFila.length; i++) {
                    if (String(dataFila[i][0]).trim() === idReset) {
                        sheetFila.getRange(i + 1, 10).setValue(data.status); // Coluna 10 (J) é o Status
                        break;
                    }
                }
            }
...
function updateMirrorResult(payload) {
    ...
    const sheetName = (type === "FETCH_GROUPS") ? "Espelho_Fila" : "Espelho_Solicitacoes";
    ...
    const data = sheet.getDataRange().getValues();
    const statusCol = (type === "FETCH_GROUPS") ? 5 : 8; // Coluna E vs Coluna H
    const errorCol = (type === "FETCH_GROUPS") ? 7 : 9;  // Coluna G vs Coluna I
    const resultCol = 6; // Para grupos (FETCH_GROUPS)
    ...
    for (let i = 1; i < data.length; i++) {
        if (String(data[i][0]).trim() === idReq) {
            if (status === "SUCESSO" || status === "GRUPOS_ENCONTRADOS") {
                ...
                if (type === "FETCH_GROUPS") sheet.getRange(i + 1, resultCol).setValue(payload.grupos);
            } else {
...
function submitMirrorExecution(data) {
    ...
    const ss = SpreadsheetApp.openById(ID_PLANILHA_GESTAO);
    let sheet = ss.getSheetByName("Espelho_Solicitacoes");
...
function doGet(e) {
    // 1. Rota de Web App (Aprovação/Reprovação)
    if (e.parameter && e.parameter.action) {
        return handleApprovalAction(e);
    }

    // 2. Rota de API JSON (Scripts)
    if (e.parameter && e.parameter.mode) {
        ...
        if (e.parameter.mode === 'get_daemon_queue') { return handleUnifiedQueue(); }
        if (e.parameter.mode === 'debug_full') { return ContentService.createTextOutput(debugSheetReader()).setMimeType(ContentService.MimeType.JSON); }
    }

    // Retorna HTML da página
    return HtmlService.createTemplateFromFile('AppsScript_Web_Index').evaluate()
        .setTitle('AD Reset Tool - Suporte Infra CDs')
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}
...
function handleApprovalAction(e) {
    ...
    for (let i = 1; i < data.length; i++) {
        if (String(data[i][0]).trim() === String(id)) {
            ...
            if (String(data[i][8]).match(/SUCESSO|CONCLU|FINALIZADO/)) {
...
function handleUnifiedQueue() {
    ...
    // 1. RESET (Solicitações) - Requer APROVAÇÃO
    const sheetReset = ss.getSheetByName("Solicitações");
    if (sheetReset) {
        const rData = sheetReset.getDataRange().getDisplayValues();
        // A partir da linha 2 (índice 1)
    ...
    // 2. FETCH GROUPS (Espelho_Fila) - NÃO Requer Aprovação (Apenas leitura)
    const sheetFila = ss.getSheetByName("Espelho_Fila");
    if (sheetFila) {
    ...
    // 3. EXECUTE MIRROR (Espelho_Solicitacoes) - Requer APROVAÇÃO
    const sheetExec = ss.getSheetByName("Espelho_Solicitacoes");
    if (sheetExec) {
        const eData = sheetExec.getDataRange().getValues();
